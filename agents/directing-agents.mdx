...
## Sharing Files With Your Agent

Instructions do not have to be text alone. When a document, spreadsheet, image, or other file would explain your request better than a description, hand the file to your agent directly.

You can share files in two ways:

- **Drop files into a session.** Drag files from your computer anywhere onto a session — new or existing — and they attach to your next message. An overlay confirms the drop, and the attached files appear alongside your instruction so the agent can read them as part of the task. Folders cannot be dropped; attach the individual files instead.
- **Upload files to your workspace.** Use the file manager in your workspace to place files directly into your project, such as brand assets, data files, or reference documents your agent should work with over time. See [uploading files to your workspace](/workspace/overview#uploading-files-to-your-workspace) for details.

Use a session attachment when the file supports a single request, such as a design mockup or an example export. Use a workspace upload when the file belongs in the project itself.

```text
Here's a spreadsheet with our current price list. Update the pricing page
so it matches these numbers exactly.
```
