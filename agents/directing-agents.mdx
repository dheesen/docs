...
Every session carries a shared plan: a single document where the agent writes out its intended approach in plain language before and during the work. Open the **Plan** tab in a session — available on both desktop and mobile — to read what the agent intends to do, and to align with your team before significant work begins.

The plan works on a draft-and-publish model. The agent (or any editor on the session) prepares changes as a draft first and publishes only when asked, so the published version always reflects an approach you have signed off on. While a newer draft is being edited, the previously published plan stays visible. Anyone viewing the session — including people following a public share link — sees only the published plan, never work-in-progress drafts.

Use the session plan when the stakes of a task justify a written approach before execution:

- **Before large features** — ask the agent to draft a plan, review it in the Plan tab, and tell the agent to publish once you agree with the approach.
- **During long-running work** — the plan updates live as it changes, so everyone following the session sees the current published intent without refreshing.
- **When collaborating** — share the session and teammates can review the same published plan, keeping feedback anchored to one agreed document rather than scattered messages.

If you fork a session to explore a different direction, the new session starts with a draft copy of the original's published plan, ready to adapt.

<Note>
  The session plan is separate from the task checklist the agent maintains while executing. The checklist tracks step-by-step progress; the plan communicates the overall approach for you and your team to review and agree on.
</Note>
