...
<Accordion title="You already use Azure, GCP, or AWS">
  ...
  For Azure routes, you can connect with **Microsoft Entra ID (Azure CLI)** instead of storing a long-lived API key. Sign in with `az login` in your workspace, then choose the Microsoft Entra ID option under **Settings → Providers → Azure** and select an Azure resource you have access to. Azotha requests short-lived access tokens on demand and never persists them, and the model picker lists the deployments available on your selected resource. The standard API-key method remains available if you prefer it.
</Accordion>
